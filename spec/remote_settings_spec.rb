RSpec.describe Celerbrake::RemoteSettings do
  let(:project_id) { 123 }
  ...
  describe ".poll" do
    ...
    context "when API returns a non-200 response" do
      ...
      it "logs error" do
        ...
        expect(Celerbrake::Loggable.instance).to have_received(:error).with(body.to_json)
      end
    end
    ...
    context "when API returns a 200 response" do
      ...
      it "doesn't log errors" do
        ...
        expect(Celerbrake::Loggable.instance).not_to have_received(:error)
        expect(stub).to have_been_requested.once
      end
    end
