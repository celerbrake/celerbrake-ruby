RSpec.describe Celerbrake::Backtrace do
  describe ".parse" do
    context "UNIX backtrace" do
      let(:parsed_backtrace) do
        ...
        [{:file=>"/home/kyrylo/code/celerbrake/ruby/spec/spec_helper.rb", :line=>23, :function=>"<top (required)>"},
         {:file=>"/opt/rubies/ruby-2.2.2/lib/ruby/2.2.0/rubygems/core_ext/kernel_require.rb", :line=>54, :function=>"require"},
         {:file=>"/opt/rubies/ruby-2.2.2/lib/ruby/2.2.0/rubygems/core_ext/kernel_require.rb", :line=>54, :function=>"require"},
         {:file=>"/home/kyrylo/code/celerbrake/ruby/spec/celerbrake_spec.rb", :line=>1, :function=>"<top (required)>"},
      ...
      it "returns a properly formatted array of hashes" do
        expect(described_class.parse(CelerbrakeTestError.new))
    ...
    context "Windows backtrace" do
      ...
      let(:ex) { CelerbrakeTestError.new.tap { |e| e.set_backtrace(windows_bt) } }
    ...
    context "JRuby Java exceptions" do
      ...
      it "returns a properly formatted array of hashes" do
        ...
        expect(described_class.parse(JavaCelerbrakeTestError.new))
    ...
    context "JRuby classloader exceptions" do
      ...
      let(:ex) { JavaCelerbrakeTestError.new.tap { |e| e.set_backtrace(backtrace) } }
    ...
    context "JRuby non-throwable exceptions" do
      ...
      let(:ex) { CelerbrakeTestError.new.tap { |e| e.set_backtrace(backtrace) } }
    ...
    context "generic backtrace" do
      context "when function is absent" do
        ...
        let(:ex) { CelerbrakeTestError.new.tap { |e| e.set_backtrace(generic_bt) } }
    ...
    context "unknown backtrace" do
      ...
      let(:ex) { CelerbrakeTestError.new.tap { |e| e.set_backtrace(unknown_bt) } }
    ...
    context "given a backtrace with an empty function" do
      let(:bt) do
        ["/celerbrake-ruby/vendor/jruby/1.9/gems/rspec-core-3.4.1/exe/rspec:3:in `'"]
      end

      let(:ex) { CelerbrakeTestError.new.tap { |e| e.set_backtrace(bt) } }

      let(:parsed_backtrace) do
        [{ file: '/celerbrake-ruby/vendor/jruby/1.9/gems/rspec-core-3.4.1/exe/rspec',
           line: 3,
           function: '' }]
      end
    ...
    context "given an Oracle backtrace" do
      ...
      it "returns a properly formatted array of hashes" do
        stub_const('OCIError', CelerbrakeTestError)
        expect(described_class.parse(ex)).to eq(parsed_backtrace)
      end
    end
    ...
    context "given an ExecJS exception" do
      ...
      it "returns a properly formatted array of hashes" do
        stub_const('ExecJS::RuntimeError', CelerbrakeTestError)
        expect(described_class.parse(ex)).to eq(parsed_backtrace)
      end
    end

    context "when code hunks are enabled" do
      before { Celerbrake::Config.instance.merge(code_hunks: true) }

      context "and when root_directory is configured" do
        before do
          Celerbrake::Config.instance.merge(root_directory: project_root_path(''))
        end
      ...
      context "and when root_directory isn't configured" do
        before do
          Celerbrake::Config.instance.merge(root_directory: nil)
          stub_const('Celerbrake::Backtrace::CODE_FRAME_LIMIT', 2)
        end
    ...
    context "when code hunks are disabled" do
      before { Celerbrake::Config.instance.merge(code_hunks: false) }
