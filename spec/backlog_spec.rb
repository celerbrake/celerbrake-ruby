RSpec.describe Celerbrake::Backlog do
  subject(:backlog) { described_class.new(sync_sender, 0.1) }

  let(:sync_sender) { Celerbrake::SyncSender.new }
  let(:error_endpoint) { '/error' }
  let(:event_endpoint) { '/event' }
  let(:promise) { an_instance_of(Celerbrake::Promise) }
  ...
  describe "#<<" do
    ...
    context "when then backlog reaches its capacity of 100" do
      before { allow(Celerbrake::Loggable.instance).to receive(:error) }
      ...
      it "logs errors" do
        ...
        expect(Celerbrake::Loggable.instance).to have_received(:error).with(
          '**Celerbrake: Celerbrake::Backlog full',
        ).twice
      end
    end
  end
