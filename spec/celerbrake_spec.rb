RSpec.describe Celerbrake do
  let(:remote_settings) { instance_double(Celerbrake::RemoteSettings) }

  before do
    allow(Celerbrake::RemoteSettings).to receive(:poll).and_return(remote_settings)
  ...
  describe ".configure" do
    ...
    it "yields the config" do
      expect do |b|
        ...
        nil
      end.to yield_with_args(Celerbrake::Config)
    end

    it "sets logger to Celerbrake::Loggable" do
      ...
      expect(Celerbrake::Loggable.instance).to eql(logger)
    end

    it "makes Celerbrake configured" do
    ...
    context "when called multiple times" do
      ...
      it "appends some default filters" do
        ...
        expect(described_class.notice_notifier).to have_received(:add_filter).with(
          an_instance_of(Celerbrake::Filters::RootDirectoryFilter),
        )
      end
    end
    ...
    context "when blocklist_keys gets configured" do
      ...
      it "adds blocklist filter" do
        ...
        expect(described_class.notice_notifier).to have_received(:add_filter)
          .with(an_instance_of(Celerbrake::Filters::KeysBlocklist))
      end

      it "initializes blocklist with specified parameters" do
        allow(Celerbrake::Filters::KeysBlocklist).to receive(:new)

        described_class.configure { |c| c.blocklist_keys = %w[password] }

        expect(Celerbrake::Filters::KeysBlocklist)
    ...
    context "when allowlist_keys gets configured" do
      ...
      it "adds allowlist filter" do
        ...
        expect(described_class.notice_notifier).to have_received(:add_filter)
          .with(an_instance_of(Celerbrake::Filters::KeysAllowlist))
      end

      it "initializes allowlist with specified parameters" do
        allow(Celerbrake::Filters::KeysAllowlist).to receive(:new)

        described_class.configure { |c| c.allowlist_keys = %w[banana] }
        expect(Celerbrake::Filters::KeysAllowlist)
    ...
    context "when root_directory gets configured" do
      ...
      it "adds root directory filter" do
        ...
        expect(described_class.notice_notifier).to have_received(:add_filter)
          .with(an_instance_of(Celerbrake::Filters::RootDirectoryFilter))
      end

      it "initializes root directory filter with specified path" do
        allow(Celerbrake::Filters::RootDirectoryFilter).to receive(:new)
        described_class.configure { |c| c.root_directory = '/my/path' }

        expect(Celerbrake::Filters::RootDirectoryFilter)
      ...
      it "adds git revision filter" do
        described_class.configure { |c| c.root_directory = '/my/path' }
        expect(described_class.notice_notifier).to have_received(:add_filter)
          .with(an_instance_of(Celerbrake::Filters::GitRevisionFilter))
      end

      it "initializes git revision filter with correct root directory" do
        allow(Celerbrake::Filters::GitRevisionFilter).to receive(:new)
        described_class.configure { |c| c.root_directory = '/my/path' }

        expect(Celerbrake::Filters::GitRevisionFilter)
          .to have_received(:new).with('/my/path')
      end
      ...
      it "adds git repository filter" do
        ...
        expect(described_class.notice_notifier).to have_received(:add_filter)
          .with(an_instance_of(Celerbrake::Filters::GitRepositoryFilter))
      end

      it "initializes git repository filter with correct root directory" do
        allow(Celerbrake::Filters::GitRepositoryFilter).to receive(:new)

        described_class.configure { |c| c.root_directory = '/my/path' }

        expect(Celerbrake::Filters::GitRepositoryFilter)
          .to have_received(:new).with('/my/path')
      end
      ...
      it "adds git last checkout filter" do
        described_class.configure { |c| c.root_directory = '/my/path' }
        expect(described_class.notice_notifier).to have_received(:add_filter)
          .with(an_instance_of(Celerbrake::Filters::GitLastCheckoutFilter))
      end

      it "initializes git last checkout filter with correct root directory" do
        allow(Celerbrake::Filters::GitLastCheckoutFilter).to receive(:new)
        described_class.configure { |c| c.root_directory = '/my/path' }

        expect(Celerbrake::Filters::GitLastCheckoutFilter)
          .to have_received(:new).with('/my/path')
      end
    end
  end
  ...
  describe ".performance_notifier" do
    it "returns a performance notifier" do
      expect(described_class.performance_notifier)
        .to be_an(Celerbrake::PerformanceNotifier)
    end
  end

  describe ".notice_notifier" do
    it "returns a notice notifier" do
      expect(described_class.notice_notifier).to be_an(Celerbrake::NoticeNotifier)
    end
  end

  describe ".deploy_notifier" do
    it "returns a deploy notifier" do
      expect(described_class.deploy_notifier).to be_an(Celerbrake::DeployNotifier)
    end
  end
  ...
  describe ".close" do
    ...
    context "when notice_notifier is undefined" do
      it "doesn't get closed (because it wasn't initialized)" do
        ...
        expect_any_instance_of(Celerbrake::NoticeNotifier).not_to receive(:close)
      end
    end
    ...
    context "when perforance_notifier is undefined" do
      it "doesn't get closed (because it wasn't initialized)" do
        ...
        expect_any_instance_of(Celerbrake::PerformanceNotifier)
