RSpec.describe Celerbrake::PerformanceNotifier do
  subject(:perf_notifier) { described_class.new }

  let(:routes) { 'https://api.celerbrake.com/api/v5/projects/1/routes-stats' }
  let(:queries) { 'https://api.celerbrake.com/api/v5/projects/1/queries-stats' }
  let(:breakdowns) { 'https://api.celerbrake.com/api/v5/projects/1/routes-breakdowns' }
  let(:queues) { 'https://api.celerbrake.com/api/v5/projects/1/queues-stats' }
  ...
  describe "#notify" do
    ...
    it "sends full performance breakdown" do
      perf_notifier.notify(
        Celerbrake::PerformanceBreakdown.new(
          method: 'DELETE',
    ...
    it "returns a promise" do
      ...
      expect(promise).to be_an(Celerbrake::Promise)
      expect(promise.value).to eq('' => '')
    end

    it "checks performance stat configuration" do
      request = Celerbrake::Request.new(
        method: 'GET', route: '/foo', status_code: 200, timing: 123,
      )
      allow(Celerbrake::Config.instance).to receive(:check_performance_options)
        .and_return(Celerbrake::Promise.new)
      perf_notifier.notify(request)
      perf_notifier.close

      expect(Celerbrake::Config.instance)
        .to have_received(:check_performance_options).with(request)
    end

    it "sends environment when it's specified" do
      Celerbrake::Config.instance.merge(performance_stats: true, environment: 'test')
    ...
    context "when an ignore filter was defined" do
      ...
      it "doesn't notify celerbrake of requests" do
        perf_notifier.notify(
          Celerbrake::Request.new(
            method: 'GET',
            route: '/foo',
      ...
      it "doesn't notify celerbrake of queries" do
        perf_notifier.notify(
          Celerbrake::Query.new(
            method: 'POST',
            route: '/foo',
      ...
      it "returns a rejected promise" do
        ...
        expect(promise.value).to eq(
          'error' => 'Celerbrake::Query was ignored by a filter',
        )
      end
    end
    ...
    context "when a filter that modifies payload was defined" do
      ...
      it "notifies celerbrake with modified payload" do
        perf_notifier.notify(
          Celerbrake::Query.new(
            method: 'POST',
            route: '/foo',
    ...
    context "when provided :timing is zero" do
      it "doesn't notify" do
        queue = Celerbrake::Queue.new(queue: 'bananas', error_count: 0, timing: 0)
  ...
  describe "#close" do
    before do
      Celerbrake::Config.instance.merge(performance_stats_flush_period: 0.1)
    end

    after do
      Celerbrake::Config.instance.merge(performance_stats_flush_period: 0)
    end
    ...
    it "logs the exit message" do
      ...
      expect(Celerbrake::Loggable.instance).to have_received(:debug).with(
        /Celerbrake::PerformanceNotifier thread pool closed/,
      )
    end
  end
