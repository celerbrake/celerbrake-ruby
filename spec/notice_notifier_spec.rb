RSpec.describe Celerbrake::NoticeNotifier do
  ...
  describe "#new" do
    describe "default filter addition" do
      before { allow_any_instance_of(Celerbrake::FilterChain).to receive(:add_filter) }

      it "appends the context filter" do
        expect_any_instance_of(Celerbrake::FilterChain).to receive(:add_filter)
          .with(instance_of(Celerbrake::Filters::ContextFilter))
        notice_notifier
      end

      it "appends the exception attributes filter" do
        expect_any_instance_of(Celerbrake::FilterChain).to receive(:add_filter)
          .with(instance_of(Celerbrake::Filters::ExceptionAttributesFilter))
        notice_notifier
      end
    end
  end
  ...
  describe "#notify" do
    ...
    it "returns a promise" do
      expect(notice_notifier.notify('ex')).to be_an(Celerbrake::Promise)
      sleep 1
    end
    ...
    context "when config is invalid" do
      before { Celerbrake::Config.instance.merge(project_id: nil) }
    ...
    context "when a notice is not ignored" do
      it "yields the notice" do
        expect { |b| notice_notifier.notify('ex', &b) }
          .to yield_with_args(Celerbrake::Notice)
        sleep 1
      end
    end
    ...
    context "when a notice is ignored via an inline filter" do
      before { notice_notifier.add_filter { raise CelerbrakeTestError } }
    ...
    context "when async sender has workers" do
      it "sends an exception asynchronously" do
        expect_any_instance_of(Celerbrake::AsyncSender).to receive(:send)
    ...
    context "when async sender doesn't have workers" do
      it "sends an exception synchronously" do
        allow_any_instance_of(Celerbrake::AsyncSender)
          .to receive(:has_workers?).and_return(false)
        expect_any_instance_of(Celerbrake::SyncSender).to receive(:send)
    ...
    context "when the provided environment is ignored" do
      ...
      it "doesn't send an notice" do
        expect_any_instance_of(Celerbrake::AsyncSender).not_to receive(:send)
        notice_notifier.notify('foo', bingo: 'bango')
      end
  ...
  describe "#notify_sync" do
    ...
    context "when the provided environment is ignored" do
      ...
      it "doesn't send an notice" do
        expect_any_instance_of(Celerbrake::SyncSender).not_to receive(:send)
        notice_notifier.notify_sync('foo', bingo: 'bango')
      end
  ...
  describe "#add_filter" do
    context "given a block" do
      it "appends a new filter to the filter chain" do
        ...
        expect_any_instance_of(Celerbrake::FilterChain)
  ...
  describe "#build_notice" do
      ...
      context "and also when it doesn't have own backtrace" do
        context "and when the generated backtrace consists only of library frames" do
          it "returns the full generated backtrace" do
            backtrace = [
              "/lib/celerbrake-ruby/a.rb:84:in `build_notice'",
              "/lib/celerbrake-ruby/b.rb:124:in `send_notice'",
            ]
            ...
            expect(notice[:errors].first[:backtrace]).to eq(
              [
                { file: '/lib/celerbrake-ruby/a.rb', line: 84, function: 'build_notice' },
                { file: '/lib/celerbrake-ruby/b.rb', line: 124, function: 'send_notice' },
              ],
            )
          end
        end
        ...
        context "and when the generated backtrace consists of mixed frames" do
          it "returns the filtered backtrace" do
            backtrace = [
              "/celerbrake-ruby/lib/celerbrake-ruby/a.rb:84:in `b'",
              "/celerbrake-ruby/lib/foo/b.rb:84:in `build'",
              "/celerbrake-ruby/lib/bar/c.rb:124:in `send'",
            ]
            allow(Kernel).to receive(:caller).and_return(backtrace)
            ...
            expect(notice[:errors].first[:backtrace]).to eq(
              [
                { file: '/celerbrake-ruby/lib/foo/b.rb', line: 84, function: 'build' },
                { file: '/celerbrake-ruby/lib/bar/c.rb', line: 124, function: 'send' },
              ],
            )
          end
        end
    ...
    context "when given exception is a Java exception", skip: true do
      before do
        allow(Celerbrake::Backtrace).to receive(:java_exception?).and_return(true)
      end
    ...
    context "when async sender is closed" do
      ...
      it "raises error" do
        expect { notice_notifier.build_notice(Exception.new('oops')) }.to raise_error(
          Celerbrake::Error,
          "Celerbrake is closed; can't build exception: Exception: oops",
        )
      end
    end
  end
  ...
  describe "#close" do
    let(:mock_async_sender) { instance_double(Celerbrake::AsyncSender) }
    let(:mock_sync_sender) { instance_double(Celerbrake::SyncSender) }

    before do
      allow(Celerbrake::AsyncSender).to receive(:new).and_return(mock_async_sender)
      allow(Celerbrake::SyncSender).to receive(:new).and_return(mock_sync_sender)
